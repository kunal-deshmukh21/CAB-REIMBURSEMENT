...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Disclaimer text that appears in every Rapido receipt — used as a
    // structural anchor to determine which address block is FROM vs TO.
    // Rapido PDFs sometimes render the pickup (FROM) address in a floating
    // element that pdfjsLib extracts AFTER the disclaimer, while the dropoff
    // (TO) address appears before it in the text stream.
    const RAPIDO_DISCLAIMER_RE = /this document is issued on request/i;
    ...
    const MONTH_YEAR_RE = new RegExp(`^(${MONTH_ALT})\\s(\\d{4})$`, "i");

    // ─── LOCATION KEYWORDS (pre-lowercased) ───────────────────────────────────
    const locationKeywords = [
        ...
        { keys: ["Sansad Marg Area", "Rajpath Area", "Rail Bhawan", "Rail Bhavan", "Central Secretariat"], short: "Rail Bhawan" },
    ].map(loc => ({ ...loc, lowerKeys: loc.keys.map(k => k.toLowerCase()) }));
    ...
    toastEl.appendChild(_toastIcon);
    toastEl.appendChild(_toastSpan);

    function showToast(msg, type = "info", duration = 3000) {
        const icons = { success: "fa-circle-check", error: "fa-circle-exclamation", info: "fa-circle-info" };
        toastEl.className = `cab-toast toast-${type}`;
        _toastIcon.className = `fa-solid ${icons[type] || icons.info}`;
    ...
    function showValidationError(title, message, rowIndex, fieldName, focusEl) {
        if (rowIndex !== undefined && fieldName !== undefined) highlightTableField(rowIndex, fieldName);
        if (focusEl) {
        ...
        return Swal.fire({
            ...
            confirmButtonColor: "#6366f1", confirmButtonText: "Got it",
            customClass: { popup: "swal-cab-popup", title: "swal-cab-title", actions: "swal-cab-actions" },
            showClass: { popup: "swal__fadeIn" }, hideClass: { popup: "swal__fadeOut" },
        });
    }
    ...
    function highlightTableField(rowIndex, fieldName) {
        ...
        let input;
        if (fieldName === "purpose") input = row.children[6]?.querySelector("input:not(.hidden)");
        else if (fieldMap[fieldName] !== undefined) input = row.children[fieldMap[fieldName] + 1]?.querySelector("input");
        if (!input) return;
    ...
    function showError(title, text) { return showValidationError(title, text); }

    // ─── VALIDATION HELPERS ───────────────────────────────────────────────────
    function parseMonthYear(value) {
        const match = (value || "").match(MONTH_YEAR_RE);
    ...
    function isValidDate(dateStr) {
        ...
        return dd >= 1 && dd <= new Date(yyyy, mm, 0).getDate();
    }
    ...
    function isValidLocation(loc) { return loc && loc.length >= 3 && loc.length <= LOCATION_MAX; }

    function isValidName(name) { return name && name.length <= NAME_MAX && /^[a-zA-Z\s._-]+$/.test(name); }

    function isValidMonthYear(value) { return MONTH_YEAR_RE.test(value); }
    ...
    if (els.dropZone) {
        ...
        els.dropZone.addEventListener("drop", e => {
            e.preventDefault(); els.dropZone.classList.remove("drag-over"); handleFiles(e.dataTransfer.files);
        });
    }
    ...
    document.addEventListener("keydown", e => {
        if (e.key === "Escape" && !els.pdfDetailView.classList.contains("hidden")) closePdfViewer();
    });

    els.cabTableBody.addEventListener("input", e => {
        if (e.target.matches(".table-input")) e.target.value = e.target.value.replace(/[<>]/g, "");
        if (e.target.matches("input[type='number']")) _debouncedUpdateTotal();
    });
    ...
    els.signatureInput.addEventListener("change", e => {
        ...
        if (!allowed.includes(file.type)) { showError("Invalid Format", "Signature must be JPG, PNG or WEBP."); return; }
        if (file.size > MAX_SIGNATURE_SIZE) { showError("File Too Large", "Signature must be under " + MAX_FILE_SIZE + "MB."); return; }
        const reader = new FileReader();
        reader.onload = ev => { els.signatureImg.src = ev.target.result; els.signaturePreview.classList.remove("hidden"); };
    ...
    new Sortable(els.receiptGrid, {
        animation: 160, ghostClass: "drag-ghost", chosenClass: "drag-chosen",
        forceFallback: false, delay: 200, delayOnTouchOnly: true, touchStartThreshold: 4,
        onEnd() {
    ...
    // ─── NAV HEIGHT — memoized ────────────────────────────────────────────────
    let _cachedNavHeight = null;
    function getNavHeight() {
        if (_cachedNavHeight !== null) return _cachedNavHeight;
        ...
        for (const sel of selectors) {
            ...
            if (el) {
                ...
                if (r.top <= 4 && r.height > 20) { _cachedNavHeight = Math.round(r.bottom); return _cachedNavHeight; }
            }
        }
        _cachedNavHeight = 0;
        return 0;
    ...
    window.viewReceipt = async function (receiptId) {
        ...
        document.documentElement.style.setProperty("--pdf-viewer-top", getNavHeight() + "px");
        ...
        try {
            ...
            const cssPageWidth = Math.min(document.documentElement.clientWidth - horizPad - 8, 760);
            ...
            for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
                ...
                canvases.push({ canvas, page, viewport, pageNum });
            }
            pagesContainer.appendChild(frag);

            for (const { canvas, page, viewport, pageNum } of canvases) {
                await page.render({ canvasContext: canvas.getContext("2d"), viewport }).promise;
                if (progressFill) progressFill.style.width = Math.round((pageNum / totalPages) * 100) + "%";
            }

            if (pagePill) pagePill.textContent = totalPages + " page" + (totalPages !== 1 ? "s" : "");
            if (progressFill) { progressFill.style.width = "100%"; setTimeout(() => { progressFill.style.width = "0%"; }, 600); }
        } catch (err) {
    ...
    function parseReceiptDate(date, time) {
        ...
        if (time) {
            ...
            if (mx) {
                hh = parseInt(mx[1]); min = parseInt(mx[2]);
    ...
    async function handleFiles(fileList) {
        ...
        if (files.length > MAX_PDF_FILES) { showError("Too Many Files", "Maximum 50 PDFs allowed at once."); return; }
        for (const file of files) {
            if (file.type !== "application/pdf") { showError("Invalid File", `"${file.name}" is not a PDF.`); return; }
            if (file.size > MAX_PDF_SIZE) { showError("File Too Large", `"${file.name}" exceeds ` + MAX_FILE_SIZE + `MB.`); return; }
        }
        ...
        try {
            const buffers = await Promise.all(files.map(f =>
                f.arrayBuffer().catch(() => { throw new Error(`Could not read "${f.name}". Try again.`); })
            ));
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (els.pdfProgressFill) els.pdfProgressFill.style.width = pct + "%";
                await processReceiptFromBuffer(files[i], buffers[i]);
        ...
        state.receipts.sort((a, b) =>
            parseReceiptDate(a.data.date, a.data.time) - parseReceiptDate(b.data.date, b.data.time)
        );
    ...
    // ─── PROCESS RECEIPT ──────────────────────────────────────────────────────
    async function processReceiptFromBuffer(file, arrayBuffer) {
        let pdf;
        ...
        state.receipts.push({
            ...
            data: {
                ...
                fromLoc: locations.fromLoc,
                toLoc: locations.toLoc,
            },
        });
    }
    ...
    // ─── LOCATION EXTRACTION (FIX: disclaimer-based structural split) ─────────
    //
    // Problem: Rapido PDFs extract address text in inconsistent order.
    // Two observed patterns:
    //
    //   Pattern A — Disclaimer BEFORE both addresses (Receipt 1 / Auto):
    //     Text: [disclaimer] → [FROM addr, ...India] → [TO addr, ...India]
    //     Result: natural order is correct (FROM first, TO second)
    //
    //   Pattern B — Disclaimer BETWEEN addresses (Receipt 2 / Car):
    //     Text: [TO addr, ...India] → [disclaimer] → [FROM addr] (no "India" suffix)
    //     The FROM address floats in a separate PDF layout box, extracted last.
    //     Result: need to swap — address AFTER disclaimer = FROM
    //
    // Fix: split the normalized text on the disclaimer, then scan each half
    // separately. If addresses are found on BOTH sides of the disclaimer,
    // the one AFTER it is always the pickup (FROM). If both are on the same
    // side, use natural order (first = FROM, second = TO).
    //
    function extractLocationsAccurately(text) {
        const normalized = text.replace(/\n/g, " ").replace(/\s+/g, " ").toLowerCase();

        const disclaimerIdx = normalized.search(RAPIDO_DISCLAIMER_RE);
        const beforeDisclaimer = disclaimerIdx >= 0 ? normalized.slice(0, disclaimerIdx) : normalized;
        const afterDisclaimer = disclaimerIdx >= 0 ? normalized.slice(disclaimerIdx) : "";

        /**
         * Find all matched location keywords in a text segment.
         * First tries splitting by "India" (catches full address blocks).
         * Falls back to a full-segment scan for addresses without a country suffix.
         */
        function findLocsIn(segment) {
            if (!segment.trim()) return [];

            const results = [];

            // Primary: split on "india" to isolate individual address blocks
            const blocks = segment.split(/india/i).map(b => b.trim()).filter(b => b.length > 25);
            for (const block of blocks) {
                let bestScore = 0, bestLocation = "";
                for (const loc of locationKeywords) {
                    let score = 0;
                    for (const k of loc.lowerKeys) {
            ...
            // Fallback: scan the full segment for addresses that have no "India" suffix
            // (e.g. "GATE NO 6, INA metro station," at end of text in Pattern B)
            if (results.length === 0 && segment.length > 25) {
                let bestScore = 0, bestLocation = "";
                for (const loc of locationKeywords) {
                    let score = 0;
                    for (const k of loc.lowerKeys) {
                        if (segment.includes(k)) score += k.length;
                    }
                    if (score > bestScore) { bestScore = score; bestLocation = loc.short; }
                }
                if (bestLocation) results.push(bestLocation);
        ...
        const locsAfter = findLocsIn(afterDisclaimer);
        const locsBefore = findLocsIn(beforeDisclaimer);

        if (DEBUG) {
            console.log("locsBefore disclaimer:", locsBefore);
            console.log("locsAfter  disclaimer:", locsAfter);
        }

        // Pattern B: addresses on opposite sides → after-disclaimer = FROM
        if (locsAfter.length > 0 && locsBefore.length > 0) {
            return { fromLoc: locsAfter[0], toLoc: locsBefore[0] };
        }

        // Pattern A: both addresses in same section → natural order
        const all = locsAfter.length > 0 ? locsAfter : locsBefore;
        return { fromLoc: all[0] || "", toLoc: all[1] || "" };
    }
    ...
    function renderReceiptCards() {
        ...
        const cards = [...els.receiptGrid.querySelectorAll(".receipt-card")];
        state.receipts.forEach((receipt, index) => {
            schedule(() => {
                ...
                if (!canvas) return;
                renderThumbnail(receipt.pdfDoc, 1, canvas).then(() => thumb?.classList.remove("loading"));
            });
        });
    }
    ...
    async function renderThumbnail(pdfDoc, pageNum, canvas) {
        try {
            ...
            ctx.fillStyle = "#e5e7eb"; ctx.fillRect(0, 0, 84, 112);
    ...
    function renderTable() {
        ...
        state.receipts.forEach((receipt, index) => {
            ...
            row.innerHTML = `
                ...
                <td><input maxlength="${LOCATION_MAX}" type="text" placeholder="From location" class="table-input" value="${receipt.data.fromLoc || ""}"></td>
                <td><input maxlength="${LOCATION_MAX}" type="text" placeholder="To location"   class="table-input" value="${receipt.data.toLoc || ""}"></td>
                ...
                <td>
                    ...
                    <input maxlength="${PURPOSE_MAX}" type="text" placeholder="Enter purpose" class="table-input hidden" style="margin-top:4px;">
                </td>`;
            frag.appendChild(row);
        });

        els.cabTableBody.appendChild(frag);
        els.uiTotalAmount.textContent = total.toFixed(2);
    ...
    function initDateTimePickers() {
        ...
        const rangeMin = parsedMonth ? new Date(parsedMonth.year, parsedMonth.monthIndex, 1) : new Date(2000, 0, 1);
        const rangeMax = parsedMonth ? new Date(parsedMonth.year, parsedMonth.monthIndex + 1, 0) : new Date(2100, 11, 31);

        els.cabTableBody.querySelectorAll(".date-input").forEach(input => {
            const fp = flatpickr(input, {
                dateFormat: "d-m-Y", allowInput: true, disableMobile: true,
                todayHighlight: true, closeOnSelect: true, minDate: rangeMin, maxDate: rangeMax,
                onOpen() { if (!fp.selectedDates.length && parsedMonth) fp.jumpToDate(rangeMin, false); },
                onClose(selectedDates) { if (selectedDates.length) input.dispatchEvent(new Event("input", { bubbles: true })); },
            });
        ...
        els.cabTableBody.querySelectorAll(".time-input").forEach(input => {
            const fp = flatpickr(input, {
                enableTime: true, noCalendar: true, time_24hr: false, dateFormat: "h:i K",
                allowInput: true, disableMobile: true, minuteIncrement: 5,
                onClose(selectedDates) { if (selectedDates.length) input.dispatchEvent(new Event("input", { bubbles: true })); },
            });
    ...
    function updateMonthField() {
        ...
        if (best) { els.monthInput.value = best; updateDatePickerRanges(); }
    }

    els.monthInput.addEventListener("input", debounce(() => {
        updateDatePickerRanges();
    ...
    window.togglePurpose = function (select) {
        select.nextElementSibling.classList.toggle("hidden", select.value !== "Other");
    };
    ...
    // ─── TOTAL UPDATE — debounced ─────────────────────────────────────────────
    const _debouncedUpdateTotal = debounce(() => {
        const total = Array.from(els.cabTableBody.rows).reduce((sum, tr) =>
            sum + (parseFloat(tr.children[5]?.querySelector("input")?.value) || 0), 0);
        els.uiTotalAmount.textContent = "Rs. " + total.toFixed(2);
    }, 80);
    window.updateTotal = _debouncedUpdateTotal;
    ...
    function updateLoadingText(msg) { if (els.loadingText) els.loadingText.textContent = msg; }
    ...
    async function generatePDF() {
        if (!state.receipts.length) { showValidationError("No Receipts", "Please upload at least one cab receipt PDF."); return; }

        const nameVal = els.empNameInput.value.trim();
        if (!isValidName(nameVal)) { showValidationError("Invalid Name", "Please enter a valid employee name (letters only).", undefined, undefined, els.empNameInput); return; }

        const monthVal = els.monthInput.value.trim();
        if (!isValidMonthYear(monthVal)) { showValidationError("Invalid Month", "Month must be like 'January 2026'.", undefined, undefined, els.monthInput); return; }

        const desigVal = els.designationInput.value.trim();
        if (!desigVal) { showValidationError("Missing Designation", "Please enter the designation.", undefined, undefined, els.designationInput); return; }

        let validationError = null, errorRow = null, errorField = null;

        Array.from(els.cabTableBody.rows).every((tr, i) => {
            const rowNum = i + 1;
            const inputs = tr.querySelectorAll("input");
            const date = inputs[0].value.trim(), time = inputs[1].value.trim();
            const from = inputs[2].value.trim(), to = inputs[3].value.trim();
            ...
            if (!date) { validationError = `Row #${rowNum}: Date is missing.`; errorRow = i; errorField = "date"; return false; }
            if (!isValidDate(date)) { validationError = `Row #${rowNum}: Invalid date. Use DD-MM-YYYY.`; errorRow = i; errorField = "date"; return false; }
            if (!time) { validationError = `Row #${rowNum}: Time is missing.`; errorRow = i; errorField = "time"; return false; }
            if (!isValidTime(time)) { validationError = `Row #${rowNum}: Invalid time. Use HH:MM AM/PM.`; errorRow = i; errorField = "time"; return false; }
            if (!from) { validationError = `Row #${rowNum}: "From" location is missing.`; errorRow = i; errorField = "from"; return false; }
            if (!isValidLocation(from)) { validationError = `Row #${rowNum}: From location must be 3-${LOCATION_MAX} characters.`; errorRow = i; errorField = "from"; return false; }
            if (!to) { validationError = `Row #${rowNum}: "To" location is missing.`; errorRow = i; errorField = "to"; return false; }
            if (!isValidLocation(to)) { validationError = `Row #${rowNum}: To location must be 3–${LOCATION_MAX} characters.`; errorRow = i; errorField = "to"; return false; }
            if (!amount || amount <= 0 || amount > MAX_BILL_AMOUNT) { validationError = `Row #${rowNum}: Amount must be between ₹1 and ₹100000.`; errorRow = i; errorField = "amount"; return false; }
            if (purposeSelect.value === "Other") {
                ...
                if (purposeVal.length < 3 || purposeVal.length > PURPOSE_MAX) {
                    validationError = `Row #${rowNum}: Purpose must be 3–${PURPOSE_MAX} characters.`;
                    errorRow = i; errorField = "purpose"; return false;
                }
            }
            return true;
        ...
        if (validationError) { showValidationError("Incomplete Details", validationError, errorRow, errorField); return; }
        ...
        try {
            ...
            const LEFT = 14, RIGHT = 14, BOTTOM = 18;
            ...
            Array.from(els.cabTableBody.rows).forEach(tr => {
                ...
                const purpose = purposeSelect.value === "Official" ? "Official" : (purposeInput.value.trim() || "Other");
                rows.push([
            ...
            doc.autoTable({
                ...
                columnStyles: {
                    ...
                    3: { cellWidth: colW.from, halign: "center" },
                    4: { cellWidth: colW.to, halign: "center" },
                    5: { cellWidth: colW.amount, halign: "center" },
                    6: { cellWidth: colW.purpose, halign: "center" },
                },
                headStyles: { fillColor: [240, 240, 240], textColor: [0, 0, 0], fontStyle: "bold", halign: "center", fontSize: 8.5, lineWidth: 0.3, lineColor: [180, 180, 180], cellPadding: cellPad },
                bodyStyles: { textColor: [30, 30, 30], fontSize: 8.5, lineWidth: 0.2, lineColor: [180, 180, 180], cellPadding: cellPad, valign: "middle", overflow: "linebreak" },
            ...
            if (y + FOOTER_HEIGHT > pageH - BOTTOM) { doc.addPage(); y = 20; }
            ...
            doc.setFont("helvetica", "bold"); doc.setFontSize(10);
            doc.text(nameVal, RIGHT_X - (SIG_W / 2) - (doc.getTextWidth(nameVal) / 2), blockTop + 5);

            doc.setFont("helvetica", "bold"); doc.setFontSize(10);
            let roleY = blockTop + 10;
            desigLines.forEach(line => {
                doc.text(line, RIGHT_X - (SIG_W / 2) - (doc.getTextWidth(line) / 2), roleY);
                roleY += 4;
            });

            roleY += 1;
            const companyWidth = doc.getTextWidth("BISAG-N");
            doc.setFont("helvetica", "bold"); doc.setFontSize(10);
            doc.text("BISAG-N", RIGHT_X - (SIG_W / 2) - (companyWidth / 2), roleY);
    ...
    async function generateMergedReceiptsPDF() {
        ...
        const buffers = await Promise.all(sortedReceipts.map(r => r.file.arrayBuffer()));
